  ...
  init(walletConnectStore, options) {
    ...
    this.walletConnector.on("session_request", async (error, payload) => {
      ...
      try {
        ...
        this.walletConnectStore.setStatus(WALLET_CONNECT_STATUS.SESSION_REQUEST)
      } catch (e) {
        console.log(e)
    ...
    this.walletConnector.on("call_request", async (error, payload) => {
      ...
      if (payload.method) {
        ...
        if (payload.method === "eth_sendTransaction") {
          this.walletConnectStore.setStatus(WALLET_CONNECT_STATUS.SEND_TRANSACTION)
        } else if (payload.method === "eth_sign") {
          this.walletConnectStore.setStatus(WALLET_CONNECT_STATUS.SIGN_TRANSACTION)
        } else if (payload.method === "personal_sign") {
          this.walletConnectStore.setStatus(WALLET_CONNECT_STATUS.SIGN_PERSONAL_MESSAGE)
        } else if (payload.method && payload.method === "eth_signTypedData") {
          this.walletConnectStore.setStatus(WALLET_CONNECT_STATUS.SIGN_TYPED_DATA)
        }
      }
    })
