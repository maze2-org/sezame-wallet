...
import {Button, Header, Text, AppScreen, AutoImage as Image} from 'components';
import {
  CONTAINER,
  NORMAL_TEXT,
  PRIMARY_BTN,
...
export function ImportWalletStep2(props: StepProps) {
  ...
  useEffect(() => {
    if (selectedWords.length === 12 || selectedWords.length === 24)
      setIsValid(false);
      ...
      const validWordsArr = wordsArray.filter(
        word =>
          ...
          ((whitelist.length > 0 && whitelist.includes(word.trim())) ||
            (whitelist.length <= 0 &&
              allowedWords.current.includes(word.trim()))),
      );
