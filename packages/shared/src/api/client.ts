...
import { exponentialBackoffFetchRetry } from './fetchRetry'
...
export class Client {
  ...
  init(nodeHost: string, explorerApiHost: string) {
    const { explorer, node } = this.getClients(nodeHost, explorerApiHost)
  ...
  private getClients(nodeHost: string, explorerApiHost: string) {
    return {
